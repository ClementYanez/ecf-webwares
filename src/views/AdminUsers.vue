      ...
      <div v-for="(user, index) in userList" :key="index">
        <div v-if="!deleteModal[index] && !modal[index] && !edit[index]" class="contain-line">
          <div class="contain-global">
            ...
            <span><strong>Adresse : </strong>{{ user.adresse }} {{ user.codePostal }} {{ user.ville }}</span>
        ...
        <div v-if="notAdmin[index]" class="alert">
          <p>Vous ne pouvez pas passer {{ user.raisonSociale }} en tant qu'utilisateur. Il doit y avoir au moins au
            administrateur.</p>
        </div>
        <div v-if="deleteModal[index]" class="confirm">
          ...
          <div class="buttons">
            ...
            <ButtonComponent text="Non, annuler" color="#333333" textColor="#f1f1f1" @click="deleteUserModal(index)" />
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    cancelEdit(i) {
      this.userListLocalStorage = JSON.parse(localStorage.getItem("user_list")) || this.$store.state.userDatabase;
  ...
  mounted() {
    this.userList = JSON.parse(localStorage.getItem("user_list")) || this.$store.state.userDatabase;
  }
};
</script>
